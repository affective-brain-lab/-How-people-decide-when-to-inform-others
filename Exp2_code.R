...
data$r_Market_change <- rescale(data$Market_change) 
...
#ONLY share
mod <- lmer(seek/share~ Instr +r_Market_change +r_uncertainty+
              (Instr+r_Market_change+r_uncertainty|ID_n),
            data=subset(data,Task1Seek==0), REML=FALSE, control = control_params)
summary(mod)


mod <- lmer(seek/share~ r_Market_change+
              (r_Market_change|ID_n),
            data=subset(data,Task1Seek==0), REML=FALSE, control = control_params)
summary(mod)
...
mod <- lmer(seek/share~ Instr +r_Market_change+
              (Instr+r_Market_change|ID_n),
            data=subset(data,Task1Seek==0), REML=FALSE, control = control_params)
summary(mod)
...
mod <- lmer(seek/share~ r_Market_change +r_uncertainty+
              (r_Market_change+r_uncertainty|ID_n),
            data=subset(data,Task1Seek==0), REML=FALSE, control = control_params)
summary(mod)
